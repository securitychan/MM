...
function renderPreviewRaw() {
  ...
  els.previewCaption.textContent = state.rows.length ? `${state.rows.length}개 행을 읽었습니다.` : "입력된 전체 행을 표시합니다.";
}
...
function drawBarChart(canvas, rows, valueKey, labelKey, options = {}) {
  ...
  const gap = rows.length <= 1 ? 0 : Math.min(options.gap ?? 8, Math.max(0, slotWidth * 0.18));
...
function drawCharts(result) {
  const dailyRows = result.days;
  const monthlyRows = result.months;
  const symbolRows = result.symbols;
  ...
  drawBarChart(els.symbolChart, symbolRows, "realizedPnl", "symbol", {
    labelFormatter: (value) => truncateLabel(value, 10),
...
function analyzeCurrentRows() {
  ...
  renderTradePreview([...state.result.trades].reverse());
  drawCharts(state.result);
  els.exportSymbol.disabled = false;
  els.exportTrades.disabled = false;
  els.previewCaption.textContent = `${trades.length}개 행 전체를 분석했습니다.`;
